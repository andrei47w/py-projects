...
from src.domain.entities import Person, Activity, write_person_binary_file, read_person_binary_file, \
    write_activity_binary_file, read_activity_binary_file
...
class Repository:
    def __init__(self):
        ...
        self.persons_file = keys['persons']
        self.activities_file = keys['activities']

        if os.path.isfile(self.activities_file) and os.path.isfile(self.persons_file):
            self._persons = read_person_binary_file(self.persons_file)
            self._activities = read_activity_binary_file(self.activities_file)
        else:
            ...
            write_person_binary_file(self.persons_file, self._persons)
            write_activity_binary_file(self.activities_file, self._activities)
    ...
    def p_storage(self):
        ...
        """
        return read_person_binary_file(self.persons_file)
    ...
    def a_storage(self):
        ...
        """
        return read_activity_binary_file(self.activities_file)
