...
class GUI:
    ...
    def __movingDrone(self, droneImage, path, markSeen=True):
        ...
        for i in range(len(path)):
            ...
            if markSeen and self.getMapSurface()[path[i][0]][path[i][1]] == 2:
                brick = pygame.Surface((20, 20))
    ...
    def __displayMap(self):
        ...
        sensorImage = pygame.image.load("extra/sensor.png")
        pathImage = self.__image()
        for i in range(len(self.getMapSurface())):
            for j in range(len(self.getMapSurface())):
                if self.getMapSurface()[i][j] == 2:
                    pathImage.blit(sensorImage, (i * 20, j * 20))
